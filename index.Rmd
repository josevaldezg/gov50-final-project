...
results_means <- HS_DR_Education %>%
  ...
  summarize(mean_espanol = mean(Español, na.rm = TRUE),
            mean_matematicas = mean(Matemáticas, na.rm = TRUE),
            mean_sociales = mean(Sociales, na.rm = TRUE),
            mean_naturales = mean(Naturales, na.rm = TRUE))
results_means <- results_means %>%
  filter(`Nivel/Modalidad` %in% c("ADULTOS FORMAL", "ARTES", "MODALIDAD GENERAL", "TECNICO PROFESIONAL", "ACADEMICOS"))

kable(results_means, caption = "Table of Results Means")
...
p <- ggplot(data_long, aes(x = reorder_within(Schooling_type, Mean_Score, Subject), y = Mean_Score, fill = Schooling_type, label = sprintf("%.2f", Mean_Score))) +
  ...
  labs(
    ...
    caption = "Source: National Examinatios, Ministry of Education Dominican Republic"
  ) +
  theme(legend.title = element_text(),
        axis.text.y = element_blank()) +
  scale_fill_manual(values = custom_colors, breaks = fill_order) +
  facet_wrap(~Subject, ncol = 1, scales = "free", labeller = labeller(Schooling_type = customize_labels_schooling, Subject = customize_labels_subject)) +
  coord_flip()
